...
def find_existing_pr(alert_name: str, changed_files: set[str]) -> str | None:
    """Check if there's already an open PR for this alert with the same changed files.
    ...
    expected_title = f"fix: address {alert_name} alert"
    ...
    for pr in resp.json():
        if pr.get("title") != expected_title:
            continue
        body = pr.get("body") or ""
        if f"**{alert_name}**" not in body or "Generated by fixer-agent" not in body:
            continue

        # Fetch the files changed in this PR and compare
...
def create_issue(alert_name: str, description: str, analysis: str) -> tuple[int, str]:
    ...
    resp = requests.post(
        ...
        json={
            "title": f"fix: address {alert_name} alert",
            "body": issue_body,
        },
    )
    resp.raise_for_status()
    data = resp.json()
    return data["number"], data.get("html_url", "")
...
def run_investigation(investigation_id: str, payload: FixRequest):
    ...
    try:
        ...
        local_changed_files = {f for f in changed_files_result.stdout.strip().splitlines() if f}

        try:
            existing_pr_url = find_existing_pr(payload.alert_name, local_changed_files)
            if existing_pr_url:
                logger.info("Found existing open PR for alert %s: %s", payload.alert_name, existing_pr_url)
                update_investigation(investigation_id, {
        ...
        pr_url = create_pr(repo_dir, branch, payload.alert_name, analysis, issue_number)
